...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        placer = self.place
        while placer != hive:
            if not placer.bees or i < self.min_range:
                placer = placer.entrance
                i += 1
            elif i > self.max_range:
                return None
            else:
                return random_or_none(placer.bees)
        # END Problem 3 and 4
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        def double_ants():
            ...
            while placer:
                if placer.ant:
                    if not placer.ant.buffed:
                        placer.ant.damage *= 2
                        placer.ant.buffed = True
                    if placer.ant.is_container and placer.ant.contained_ant and not placer.ant.contained_ant.buffed:
                        placer.ant.contained_ant.damage *= 2
                        placer.ant.contained_ant.buffed = True
                placer = placer.exit
    ...
    def reduce_armor(self, amount):
        ...
        if not self.armor and self.true_queen:
            bees_win()
        # END Problem 13
